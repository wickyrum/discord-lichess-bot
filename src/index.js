  ...
  fields: [
    {
      ...
      value:
        "Creates a new lichess challenge. Use subcommands to specify the type of challenge (rapid, blitz, bullet) and set the clock and increment times.",
    },
    {
      name: "/help",
...
async function challengeHandler(interaction) {
  ...
  if (interaction.commandName === "create-challenge") {
    ...
    const lichessObj = await lichessCall(
      challenged,
      subcommand,
      clock,
      increment,
    ...
    await interaction.reply(
      `${subcommand} match has been created, here is the link ${lichessObj[0]}`,
    );
